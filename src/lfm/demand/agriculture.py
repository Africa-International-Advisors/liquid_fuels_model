"""Agricultural diesel demand. v1 status: HELD (ZAF only).

Compute (HELD pattern, see ``_held.py``):
    volume(t) = base_year_volume * (gdp_pc(t) / gdp_pc(base_year))^elasticity

Low GDP elasticity (~0.2) reflects that agricultural diesel tracks planted
hectares and rebate policy more than economic growth. v1 uses GDP as a
placeholder driver; replacing with hectares-planted is a planned enhancement.
"""
...
def compute_country_annual(
    ...
    base_block = provider.get("agriculture", "base_year_volume", run).value.get(iso3) or {}
    base_volume = base_block.get("value")
    base_year = base_block.get("year")
    elasticity = provider.get("agriculture", "elasticity", run).value.get(iso3)
